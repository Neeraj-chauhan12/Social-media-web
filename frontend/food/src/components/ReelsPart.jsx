...
const ReelsPart = ({
  ...
  return videos.length === 0 ? (
    <div className="h-screen w-screen flex items-center justify-center bg-black text-white text-xl font-semibold">
      {emptyMessage}
    </div>
  ) : (
    <div
        ...
        >
          ...
          <div className="absolute right-4 bottom-32 flex flex-col gap-4 items-center z-20">
            <div className="flex flex-col items-center">
              <button
                onClick={onLike ? () => onLike(video) : undefined}
                className="bg-black/60 rounded-full p-2 mb-1 border border-gray-700"
              >
                <FaRegHeart className="w-8 h-8 text-white" />
              </button>
              <span className="text-white text-xs">
                {video.likeCount ?? video.likesCount ?? video.likeCount ?? 0}
              </span>
            </div>
            ...
            <div className="flex flex-col items-center">
              <button
                onClick={onSave ? () => onSave(video) : undefined}
                className="bg-black/60 rounded-full p-2 mb-1 border border-gray-700"
              >
                <FaRegBookmark className="w-8 h-8 text-white" />
              </button>
              <span className="text-white text-xs">
                {video.saveCount ?? video.saveCount ?? 0}
              </span>
            </div>
          </div>
          ...
          <div className="absolute bottom-16 left-0 w-full flex flex-col items-center z-10 px-4 pb-8 pointer-events-none">
            <div className="w-full max-w-lg">
              <p
                className="text-white text-2xl  font-medium line-clamp-2  rounded px-2 hover:bg-gray-700/50 transition-colors"
                style={{
                  display: "-webkit-box",
                  WebkitLineClamp: 2,
              ...
              <p className="px-2 rounded  text-white font-semibold ">
                {video?.description}
              </p>
            </div>
          </div>
